...
const DEFAULT_PICKER_METHODS = ['nip07', 'amber', 'remote-signet', 'local-signet', 'bunker', 'nostrconnect', 'nsec'];
 ...
 */
function likelySignetKey(mobile) {
...
async function waitForNip07Operation(operation, cancelled, timeoutMs, phase) {
    let timeoutId;
...
async function runNip07Flow(refs, opts) {
  ...
  `;
    ...
    const ticker = window.setInterval(() => {
        elapsed += 1;
        if (elapsedEl)
            elapsedEl.textContent = `Waiting for ${waitStage} (${elapsed}s)…`;
    }, 1000);
    ...
    try {
        ...
        const signer = await waitForNip07Operation(createNip07Signer(), cancelled, opts.timeout, 'get-public-key');
        if (!signer)
            return null;
        elapsed = 0;
        waitStage = 'signature approval';
        if (elapsedEl)
            elapsedEl.textContent = 'Waiting for signature approval…';
        const authEvent = await waitForNip07Operation(signer.signEvent({
            kind: 21236,
            content: '',
            tags: [
        ...
        }), cancelled, opts.timeout, 'sign-event');
        if (!authEvent) {
            try {
                await signer.close();
            }
...
async function runNsecFlow(refs, opts) {
  ...
  `;
    return new Promise(resolve => {
        ...
        const passwordRow = refs.dialog.querySelector('#signet-login-nsec-password-row');
        const password = refs.dialog.querySelector('#signet-login-nsec-password');
        ...
        const syncPasswordRow = () => {
            const encrypted = isEncryptedNsec(input?.value ?? '');
            if (passwordRow)
                passwordRow.hidden = !encrypted;
            if (status)
                status.textContent = '';
        };
        ...
        const submit = () => {
            ...
            if (!value.trim()) {
                if (status)
                    status.textContent = 'Please paste an nsec or ncryptsec.';
                return;
            }
            ...
            if (encrypted && !password?.value) {
                ...
                if (status)
                    status.textContent = 'This key is password-protected. Enter its password.';
                password?.focus();
                return;
            }
            try {
